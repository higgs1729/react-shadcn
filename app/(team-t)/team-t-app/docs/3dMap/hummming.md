...
| ファイル | 責務 |
| --- | --- |
| `team-t-world-overlay.tsx` | 全面オーバーレイ。`exploring`/`briefing`/`playing` の3モードを所有。HUD(コイン残高・閉じる・操作ヒント)。キャンバスを `next/dynamic` で遅延読込。フォールバック一覧も持つ |
...
2. WASD/矢印で移動、カメラがラグ付きで追従(reduceMotion 時はラグとスウェイを切る)
3. 筐体から 3.5 以内で看板が点灯し、3D空間内のラベルにゲーム名・コスト・最大報酬
4. `E` / クリック → **briefing** パネル(現行 footer が持っていた説明+プレイボタン相当)

...
test.describe("golden flow: dryrun-saas-ops-01", () => {
  test("login route renders the login screen", async ({ page }) => {
    ...
    expect(
      response?.ok(),
      "expected a successful navigation response"
    ).toBeTruthy()
  ...
  test("overview route renders the dashboard screen", async ({ page }) => {
    const response = await page.goto(
      "/studio/flows/dryrun-saas-ops-01/overview"
    )
    expect(
      response?.ok(),
      "expected a successful navigation response"
  ...
  test("invoice-list route renders the collection screen (default state)", async ({
    page,
  }) => {
    const response = await page.goto(
      "/studio/flows/dryrun-saas-ops-01/invoice-list"
    )
    expect(
      response?.ok(),
      "expected a successful navigation response"
  ...
  test("invoice-list route exposes the empty state", async ({ page }) => {
    const response = await page.goto(
      "/studio/flows/dryrun-saas-ops-01/invoice-list?state=empty"
    )
    expect(
      response?.ok(),
      "expected a successful navigation response"
  ...
  test("invoice-list route exposes the error state", async ({ page }) => {
    const response = await page.goto(
      "/studio/flows/dryrun-saas-ops-01/invoice-list?state=error"
    )
    expect(
      response?.ok(),
      "expected a successful navigation response"
  ...
  test("invoice-list route exposes the loading state", async ({ page }) => {
    const response = await page.goto(
      "/studio/flows/dryrun-saas-ops-01/invoice-list?state=loading"
    )
    expect(
      response?.ok(),
      "expected a successful navigation response"

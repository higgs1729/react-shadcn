...
- preview画像(`game-previews/`・`api-page-previews/` の11枚)だけはこの規則の外にあり、Cloudflare R2 から配信する。URLは `lib/team-t-app/asset-base.ts` の定数を基点に組み立て、実在は `scripts/asset-manifest.json` を通じて `npm run validate` が検証する(2026-08-06、`data-and-risk.md` の Asset hosting)
